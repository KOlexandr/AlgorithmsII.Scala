...
class Interval(val left: Double, val right: Double) {
  ...
  def / (that: Interval) =
    ...
    else this * new Interval(1/that.left, 1/that.right)
  ...
  def mu = math.min(math.abs(left), math.abs(right))

  def s = (math.abs(left) + math.abs(right)) / 2
}